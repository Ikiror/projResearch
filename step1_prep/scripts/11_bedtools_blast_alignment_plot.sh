...
#SBATCH --output=/data/users/aikiror/researchProject/pipeline/step1_prep/logFiles/11_bedtools_blast_alignment_plot/output_%A_%a.o
#SBATCH --error=/data/users/aikiror/researchProject/pipeline/step1_prep/logFiles/11_bedtools_blast_alignment_plot/error_%A_%a.e
#SBATCH --partition=pibu_el8

#to run: sbatch path/to/file
#array job to create a bed file, extract sequence, compare sequence window to blast database, plot alignment per gene
...
OUTPUTDIR="${WORKDIR}/step1_prep/outputFiles/11_bedtools_blast_alignment_plot"
...
#info for gene window based on current array task
seqName=`awk -v line=$SLURM_ARRAY_TASK_ID 'NR==line{print $1; exit}' $SAMPLELIST` #name of contig
start=`awk -v line=$SLURM_ARRAY_TASK_ID 'NR==line{print $2; exit}' $SAMPLELIST` #geneic window start
stop=`awk -v line=$SLURM_ARRAY_TASK_ID 'NR==line{print $3; exit}' $SAMPLELIST` #geneic window stop
geneName=`awk -v line=$SLURM_ARRAY_TASK_ID 'NR==line{print $4; exit}' $SAMPLELIST` #gene name
geneID=`awk -v line=$SLURM_ARRAY_TASK_ID 'NR==line{print $5; exit}' $SAMPLELIST` #varia id
